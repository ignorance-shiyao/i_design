<!--
  由 packages/vue/scripts/convert.mjs 从 src/components/IDrawerForm.vue 转换而来。
  差异仅在 Vue 2 的语法约束，行为保持一致。
-->
<script setup lang="ts">
/**
 * 抽屉里的表单壳（astra.md 的 B09）。
 *
 * 与 IFormPage 是同一套判断（logic/formhost.ts），换了个容器。浮层多出来的
 * 那件事是：**关闭这个动作本身要被离开保护拦住**。点遮罩、按 Esc、点右上角
 * 的叉，在改了一半的表单上都等于「放弃刚才填的东西」，而这三条路此前是
 * 直接把抽屉关掉的——用户按 Esc 只是想收起键盘，二十个字段就没了。
 *
 * 抽屉与弹窗的分工：内容高、字段多、需要一边看列表一边填的用抽屉；
 * 三五个字段、填完就走的用弹窗。两者的判断与操作条完全一样。
 */
...
const emit = defineEmits<{ (e: 'input', open: boolean): void; (e: 'inputs', values: Record<string, unknown>): void; (e: 'submit', values: Record<string, unknown>): void; (e: 'cancel'): void; (e: 'reset', values: Record<string, unknown>): void }>()
...
/** IDrawer 的关闭（遮罩、Esc、右上角的叉）都走这里，先问一句再关 */
