<!--
  由 packages/vue/scripts/convert.mjs 从 src/components/IPromptInput.vue 转换而来。
...
const emit = defineEmits<{ (e: 'input', a0: string): void; (e: 'submit', a0: string): void; (e: 'stop'): void; (e: 'attach'): void; (e: 'removeAttachment', a0: number): void; (e: 'pick', option: PromptOption, symbol: string): void }>()
...
const canSend = computed(() => !props.disabled && !over.value && props.value.trim().length > 0)
...
watch(() => props.value, resize, { immediate: true })
...
function pick(option: PromptOption) {
  ...
  const next = applyMention(props.value, trigger.value, option.label, caret)
