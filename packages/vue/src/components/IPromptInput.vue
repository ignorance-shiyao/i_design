...
const emit = defineEmits<{ (e: 'input', a0: string): void; (e: 'submit', a0: string): void; (e: 'stop'): void; (e: 'attach'): void; (e: 'removeAttachment', a0: number): void }>()
...
<template>
  <div class="i-prompt" :class="{ 'is-focused': focused, 'is-disabled': disabled }">
    <div v-if="attachments.length" class="i-prompt__attachments">
      <span v-for="(file, index) in attachments" :key="file.name + index" class="i-prompt__file">
        <IIcon name="file" :size="12" />
        {{ file.name }}
