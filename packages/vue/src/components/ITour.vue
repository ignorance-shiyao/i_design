<!--
  由 packages/vue/scripts/convert.mjs 从 src/components/ITour.vue 转换而来。
...
const emit = defineEmits<{ (e: 'input', a0: number): void; (e: 'finish'): void; (e: 'skip'): void }>()
...
const step = computed(() => props.steps[props.value] ?? null)
const isLast = computed(() => props.value === props.steps.length - 1)
...
watch(() => props.value, locate, { immediate: true })
...
function next() {
  const target = tourNext(props.value, props.steps.length)
  emit('input', target)
  if (target === -1) emit('finish')
}

function prev() {
  emit('input', tourPrev(props.value))
}
    ...
    <div v-if="step" class="i-tour" role="dialog" aria-modal="true">
      ...
      >
        ...
        <footer class="i-tour__foot">
          ...
          <span class="i-tour__count">{{ value + 1 }} / {{ steps.length }}</span>
          <div class="i-tour__actions">
            <IButton v-if="value > 0" size="sm" variant="text" @click="prev">上一步</IButton>
