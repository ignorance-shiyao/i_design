<!--
  由 packages/vue/scripts/convert.mjs 从 src/components/IChartSunburst.vue 转换而来。
...
const emit = defineEmits<{ (e: 'select', id: string): void; (e: 'focus', id: string): void }>()
    ...
    <ol class="i-sunburst__data" aria-label="层级数据">
      <li v-for="node in view" :key="node.id" :style="{ paddingInlineStart: `${node.depth * 16}px` }">
        <button
          ...
          :aria-pressed="String(selectedId === node.id)"
