<!--
  由 packages/vue/scripts/convert.mjs 从 src/components/ICheckbox.vue 转换而来。
...
const props = withDefaults(
  ...
  { value: false, value: undefined, disabled: false, indeterminate: false }
)

const emit = defineEmits<{ (e: 'input', a0: boolean): void; (e: 'change', a0: boolean): void }>()

const group = inject(checkboxGroupKey, null)

const checked = computed(() =>
  group && props.value !== undefined ? group.value.value.includes(props.value) : props.value
)
...
<template>
  ...
  >
    <input
      ...
      :checked="checked"
      :disabled="disabled"
      :aria-checked="String(indeterminate ? 'mixed' : checked)"
      @change="toggle"
    />
