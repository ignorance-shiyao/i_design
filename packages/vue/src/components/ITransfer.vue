<!--
  由 packages/vue/scripts/convert.mjs 从 src/components/ITransfer.vue 转换而来。
...
const sides = computed(() => splitSides(props.items, props.value))
...
function move(to: TransferSide) {
  ...
  emit('input', moveKeys(props.items, props.value, moving, to))
