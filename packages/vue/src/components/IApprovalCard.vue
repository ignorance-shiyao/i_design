<!--
  由 packages/vue/scripts/convert.mjs 从 src/components/IApprovalCard.vue 转换而来。
...
const emit = defineEmits<{ (e: 'complete', answers: Record<string, ApprovalAnswer>): void; (e: 'renew'): void; (e: 'review'): void; (e: 'close'): void }>()
  ...
  <section v-if="current" class="i-agent-card i-approval" role="group" :aria-label="current.title">
    ...
    <div class="i-approval__options" :role="current.multiple ? 'group' : 'radiogroup'">
      <button
        ...
        :aria-checked="String(selected.includes(option.value))"
