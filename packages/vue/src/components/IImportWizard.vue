<!--
  由 packages/vue/scripts/convert.mjs 从 src/components/IImportWizard.vue 转换而来。
...
const emit = defineEmits<{ (e: 'input', mapping: ColumnMapping): void; (e: 'dry-run'): void; (e: 'submit'): void; (e: 'download', csv: string): void }>()

const issues = computed(() => mappingIssues(props.value, props.fields))
...
const sampleOf = (field: string) =>
  props.sources.find((s) => s.key === props.value[field])?.sample ?? ''
...
function pick(field: string, value: unknown) {
  ...
  emit('input', assignMapping(props.value, field, one ? String(one) : null))
}
</script>
