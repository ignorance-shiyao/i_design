<!--
  由 packages/vue/scripts/convert.mjs 从 src/components/IPageHeader.vue 转换而来。
...
<template>
  <header class="i-page-header">
    ...
    <div class="i-page-header__main">
      ...
      <p v-if="subtitle || $scopedSlots.subtitle" class="i-page-header__subtitle">
        <slot name="subtitle">{{ subtitle }}</slot>
      </p>
    </div>

    <div v-if="$scopedSlots.extra" class="i-page-header__extra"><slot name="extra" /></div>
    <div v-if="$scopedSlots.default" class="i-page-header__content"><slot /></div>
  </header>
</template>
