<!--
  由 packages/vue/scripts/convert.mjs 从 src/components/IChartAdjacency.vue 转换而来。
...
const emit = defineEmits<{ (e: 'select', row: number, col: number): void }>()
            ...
            <tr v-for="(row, rowIndex) in model.cells" :key="model.nodes[rowIndex].id">
              ...
              <td v-for="(cell, colIndex) in row" :key="`${cell.rowId}-${cell.columnId}`">
                <button
                  ...
                  :aria-pressed="String(selectedRow === rowIndex && selectedCol === colIndex ? 'true' : 'false')"
