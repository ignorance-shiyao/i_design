<!--
  由 packages/vue/scripts/convert.mjs 从 src/components/ITreeSelect.vue 转换而来。
...
const display = computed(() => {
  ...
  return props.showPath
    ? labelPath(entities.value, props.value).join(props.separator)
    : (entities.value.get(props.value)?.node.label ?? props.value)
})
