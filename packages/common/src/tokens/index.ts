...
export const palette: Record<string, Palette> = {
  ...
  gray: {
     ...
     * 视觉上仍明显比正文轻，但读得清楚。取值以「在所有底色上都过 4.5」为准，
     * 只看白底的话，浅灰卡片里的说明文字会差一点点过不去。
     *
     * 后来又压了一档到 #686d7d：#6b7080 在第三档底色 #eef0f5（填充面板、
     * 演示区）上只有 4.33，而第三级文字最常出现的地方恰恰就是这种面板。
     * 现在三档底色上分别是 5.16 / 4.85 / 4.52，都过线。
     */
    60: '#686d7d',
    70: '#575d6c',
    80: '#252b3a',
...
export const motion = {
   ...
   * 骨架扫光的一个来回。
   *
   * 它是唯一一个「无限循环」的动效，因此单列一个令牌：主题面板调慢动效时，
   * 一个仍按原速来回扫的骨架屏会格外扎眼。写死在样式里就调不到。
   */
  skeleton: '1400ms'
} as const
