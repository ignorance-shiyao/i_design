...
const emit = defineEmits<{
  'update:values': [value: FineTuneValues]
...
function set(field: FineTuneField, value: FineTuneValue) {
  ...
  emit('update:values', { ...props.values, [field.key]: clampFieldValue(field, value) })
}

function revert(field: FineTuneField) {
  emit('update:values', resetField(props.original, props.values, field.key))
}
</script>
        ...
        <div class="i-finetune__control">
          <template v-if="field.kind === 'number'">
            ...
            <input
              ...
              @input="set(field, Number(($event.target as HTMLInputElement).value))"
            />
            <input
              ...
              @change="set(field, Number(($event.target as HTMLInputElement).value))"
            />
          </template>
          ...
          <input
            ...
            :disabled="field.disabled"
            :aria-label="field.label"
            @change="set(field, ($event.target as HTMLInputElement).value)"
          />
        </div>
