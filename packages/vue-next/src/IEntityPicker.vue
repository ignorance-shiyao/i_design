...
const emit = defineEmits<{
  'update:modelValue': [chosen: EntityOption[]]
...
const summary = computed(() => pickerSummary(props.modelValue, props.max, props.unit))
/** 已选里不在当前页的那些也要显示——不然翻一页就看不见自己选了谁 */
const offPage = computed(() => offPageChosen(props.modelValue, props.page))
...
<template>
  <div class="i-entity-picker">
    ...
    <ul v-if="modelValue.length" class="i-entity-picker__chosen">
      <li
      ...
      >
        ...
        <button
          ...
          @click="emit('update:modelValue', removePick(modelValue, item.id))"
        >
    ...
    <ul v-else class="i-entity-picker__list">
      <li v-for="row in rows" :key="row.id">
        <button
          ...
          @click="emit('update:modelValue', togglePick(input, row.id))"
        >
          <IIcon
