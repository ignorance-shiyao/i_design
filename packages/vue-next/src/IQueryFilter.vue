...
const emit = defineEmits<{
  'update:modelValue': [state: QueryState]
  /** 条件变化后的参数记录，页面拿去写地址栏或路由参数 */
  change: [payload: { state: QueryState; params: Record<string, string> }]
}>()
...
const active = computed(() => activeCount(props.modelValue.values))
...
function onChange(name: string, value: FilterValue) {
  push(changeFilter(props.modelValue, name, value))
}

function onRange(name: string, part: 'from' | 'to', raw: string) {
  const current = (props.modelValue.values[name] ?? {}) as FilterRange
  onChange(name, { ...current, [part]: raw || undefined })
}

const rangeOf = (name: string) => (props.modelValue.values[name] ?? {}) as FilterRange
const textOf = (name: string) => (props.modelValue.values[name] as string) ?? ''
const listOf = (name: string) => (props.modelValue.values[name] as string[]) ?? []
  ...
  <section class="i-query" :aria-label="'查询筛选'">
    ...
    <div v-if="quickFilters.length" class="i-query__quick">
      <button
        ...
        :class="{ 'is-on': matchQuickFilter(modelValue.values, quick) }"
        :aria-pressed="matchQuickFilter(modelValue.values, quick)"
        @click="push(applyQuickFilter(modelValue, quick))"
      >
        {{ quick.label }}
      </button>
    </div>
    ...
    <div class="i-query__grid">
      <label v-for="field in shownFields" :key="field.name" class="i-query__field">
        ...
        <IInput
          ...
          :model-value="textOf(field.name)"
          :placeholder="field.placeholder"
          @update:model-value="(v: string) => onChange(field.name, v)"
        />
        ...
        <ISelect
          ...
          @update:model-value="(v: unknown) => onChange(field.name, Array.isArray(v) || v == null ? '' : String(v))"
        />
        ...
        <span v-else-if="field.kind === 'multi-select'" class="i-query__tags">
          <button
            ...
            :aria-pressed="listOf(field.name).includes(option.value)"
        ...
        <span v-else class="i-query__range">
          <IInput
            :model-value="rangeOf(field.name).from ?? ''"
            :type="field.kind === 'date-range' ? 'date' : 'number'"
            @update:model-value="(v: string) => onRange(field.name, 'from', v)"
          />
          <span class="i-query__range-sep" aria-hidden="true">–</span>
          <IInput
            :model-value="rangeOf(field.name).to ?? ''"
            :type="field.kind === 'date-range' ? 'date' : 'number'"
            @update:model-value="(v: string) => onRange(field.name, 'to', v)"
          />
        </span>
      </label>

      <div class="i-query__actions">
        <IButton v-if="active > 0" size="sm" @click="push(clearFilters(modelValue))">
          清空条件（{{ active }}）
        </IButton>
        <button
