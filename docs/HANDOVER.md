...
- 本轮已完成并勾掉的：B01 AppShell、B03 QueryFilter、B04+B05 ProTable、
  B07 批量操作栏、B09 表单页形态、B10 人员选择、B06 树表、B11 导入向导与导出任务、B18 任务中心、B14 评论线程、B15 看板、B12 详情页、
  F06 运行状态、F08 确认失效、
  B08 SchemaForm、C03 图标填充变体、C06 插画矩阵、D04 分布图、D13 轴系、
  G02 示例门户、G03 ERP 第一切片（本轮换成组件后转正）。

**下一件事**：**B06 剩下的多级表头与单元格合并** —— 树表与分组汇总已完成，
剩下的是表头的结构问题（跨列的表头怎么排、合并单元格怎么与排序共存），
不是行的状态问题。之后是 H02～H04/H06/H07 的端到端回归与发布演练；
H06 没做完之前不要宣称「可发布」。
B15 已完成（拖动/菜单/键盘三条路径同一个 moveCard，落不下的当场写理由）。
