...
**D05 层级三件套已完成**：共享汇总契约 `logic/hierarchy.ts` + 旭日 + Icicle，
各端齐备，见 `docs/D05-HIERARCHY.md`。父子差额单列为「未细分」而不是替读者挑一个数，
占全体与占上级两个分母都报，叶子的 null 是缺失不是零。

**下一件仓库内可做的仍是 D05**：留存与贡献/流量平衡。留存要先定
「分母是哪一批人、第几期」的契约；贡献/流量平衡要同时表达正负贡献，
不能沿用层级那套「负值一律排除」，得单独定口径。
