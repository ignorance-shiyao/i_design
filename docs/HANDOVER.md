...
- 本轮已完成并勾掉的：B01 AppShell、B03 QueryFilter、B04+B05 ProTable、
  B07 批量操作栏、B09 表单页形态、B10 人员选择、B06 树表、B11 导入向导与导出任务、B18 任务中心、B14 评论线程、B12 详情页、
  F06 运行状态、F08 确认失效、
  B08 SchemaForm、C03 图标填充变体、C06 插画矩阵、D04 分布图、D13 轴系、
  G02 示例门户、G03 ERP 第一切片（本轮换成组件后转正）。

**下一件事**：**B15 看板与人员泳道的拖动** —— `ICalendar`、`IChartGantt` 在，
缺拖动。关键是「拖动与菜单移动结果一致」（同一次移动，两条路径必须落到
同一个结果，否则用户会以为其中一条坏了），以及不可拖的项要当场说明原因。
B14 已完成（未读分隔线钉死不动、活动记录与对话按时间穿插）。
