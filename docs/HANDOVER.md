...
**下一件事**：`docs/astra.md` §10 的「下一步的顺序」第 5 条 —— **F06 / F08 的缺口**。
两处都是「状态在 reducer 里有了，但没在界面上说出来」：

- **F06**：生成生命周期的「排队中」「连接中」两态没有 UI 呈现。
  用户看到的是一片静止，不知道是卡住了还是在排队。
- **F08**：`IApprovalCard` 缺「过期」与「版本失效」的决定路径 ——
  人离开一会儿回来，那个待确认的动作可能已经不该再执行了，界面要说清楚。
