...
- 本轮已完成并勾掉的：B01 AppShell、B03 QueryFilter、B04+B05 ProTable、
  B07 批量操作栏、B09 表单页形态、B10 人员选择、B06 树表、B11 导入向导与导出任务、B18 任务中心、B12 详情页、
  F06 运行状态、F08 确认失效、
  B08 SchemaForm、C03 图标填充变体、C06 插画矩阵、D04 分布图、D13 轴系、
  G02 示例门户、G03 ERP 第一切片（本轮换成组件后转正）。

**下一件事**：**B14 评论的未读定位** —— `IComment` 在，缺「从上次读到的地方
继续」与活动记录。关键是未读位置按人存，且新评论插进来时不跳位。
之后是 B15（看板与人员泳道的拖动：拖动与菜单移动结果必须一致，
不可拖的项要说明原因）。
B18 已完成（命令面板发起 → 任务中心追踪 → 通知定位到业务对象，三段打通）；
B06 只剩多级表头与单元格合并——那是表头的结构问题，不是行的状态问题，
排在 B14 / B15 之后。

F06、F08、B07、B09、B10、B12、G03 与 B11（导入 + 导出两半）已完成
（见下面「这一轮做过什么」）。
