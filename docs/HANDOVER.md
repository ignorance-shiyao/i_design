...
**下一件仓库内可做的是 D07 实时滑窗或 D08 邻接矩阵**，两件都不依赖别的人或环境。
D07 先定「窗口按时间还是按条数、迟到的点算进哪一窗」；D08 的难点在排序口径
与稀疏矩阵里空格的语义（没有边，还是没统计到）。都是同一类问题：
口径不写死在共享逻辑里，两个端迟早各算各的。
