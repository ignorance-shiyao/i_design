...
**下一件事**：**H02 关键流程 E2E**。它原本卡在 G05/G08，现在 G08 已完成，
ERP（G03）也在，两条业务链够写第一版端到端回归了；OA（G05）还没有，
所以这一版只能覆盖 ERP 与 Agent 两条，别把它称作完整的端到端回归。
之后是 **H04 的补完 / H06 / H07** —— 端到端业务链回归、灰度与回滚演练、
渠道发布演练。**H06 没做完之前不要宣称「可发布」**（它卡在用户手动配
NPM_TOKEN，见第 6 节）。这几条与前面那些不同：它们要的不是新组件，
而是把已有的东西连起来跑一遍，并且证明出问题时退得回去。
