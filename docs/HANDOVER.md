...
- 本轮已完成并勾掉的：B01 AppShell、B03 QueryFilter、B04+B05 ProTable、
  B07 批量操作栏、B09 表单页形态、B10 人员选择、B11 导入向导与导出任务、B12 详情页、
  F06 运行状态、F08 确认失效、
  B08 SchemaForm、C03 图标填充变体、C06 插画矩阵、D04 分布图、D13 轴系、
  G02 示例门户、G03 ERP 第一切片（本轮换成组件后转正）。

**下一件事**：**B06 树表与分组汇总** —— `ITable` 目前只有基础排序与虚拟滚动。
这条的关键是「排序与展开不能丢选择」：换个排序、收起一个分组之后，
勾上的那几行必须还在，而且「全选」在树里选中的到底是哪些行要说得出来。
之后是 B14 / B15 / B18 的补完（评论未读定位、看板与人员泳道拖动、异步任务中心）。
