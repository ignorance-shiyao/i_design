...
**下一件事**：`docs/astra.md` §10 的「下一步的顺序」第 9 条 —— **G03 换成组件**。
ERP 示例里的列表、新建与详情三页还是手写的，B04/B05/B07/B09/B12 都已就位，
换掉它们之后 G03 才谈得上从 🟡 转正。注意示例站用的是 `packages/vue-next`，
改了 `src/components` 要先 `npm run build:vue-next`（坑表里第三条）。

F06、F08、B07、B09 与 B12 已完成（见下面「这一轮做过什么」）。
