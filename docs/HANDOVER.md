...
- 本轮已完成并勾掉的：B01 AppShell、B03 QueryFilter、B04+B05 ProTable、
  B07 批量操作栏、B09 表单页形态、B10 人员选择、B12 详情页、
  F06 运行状态、F08 确认失效、
  B08 SchemaForm、C03 图标填充变体、C06 插画矩阵、D04 分布图、D13 轴系、
  G02 示例门户、G03 ERP 第一切片（本轮换成组件后转正）。

**下一件事**：`docs/astra.md` §10 的「下一步的顺序」第 11 条 —— **B11
ImportWizard / ExportJob**。手里的料已经齐了：mock API 有 `importOrders` 的
部分失败语义，B07 的 `bulkOutcome` / `mergeOutcome` 直接能接，B09 的 StepForm
正好是导入向导的骨架。难点是「预校验不写业务数据」与「列映射可回退」——
预校验一旦落库，用户取消之后数据已经脏了。之后是 B06 树表。

F06、F08、B07、B09、B10、B12 与 G03 已完成（见下面「这一轮做过什么」）。
