...
- 本轮已完成并勾掉的：B01 AppShell、B03 QueryFilter、B04+B05 ProTable、
  B07 批量操作栏、B09 表单页形态、F06 运行状态、F08 确认失效、
  B08 SchemaForm、C03 图标填充变体、C06 插画矩阵、D04 分布图、D13 轴系、
  G02 示例门户、G03 ERP 第一切片（判 🟡，原因见下）。

**下一件事**：`docs/astra.md` §10 的「下一步的顺序」第 8 条 —— **B12 DetailPage**。
它要解决的是「这条记录现在能做什么」：动作随状态变化、没权限的动作要说清
为什么而不是藏起来、以及从列表点进来之后的上一条 / 下一条。
做完它，G03 里手写的那几页才换得掉，G03 也才谈得上从 🟡 转正。

F06、F08、B09 与 B07 已完成（见下面「这一轮做过什么」）。
