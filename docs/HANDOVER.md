...
**下一件事**：**H02 第一版端到端回归**。它原本卡在 G05/G08，现在 G08 已完成、
ERP（G03）也在，两条业务链够写第一版了。**但 OA（G05）还没有，所以这一版
不能叫「完整的端到端回归」**——命名上要如实，否则下一个人会以为这条已经做完。
要测最终业务结果（单子真的到了那个状态、产物真的产出了），不是「按钮存在」；
失败 / 权限 / 冲突的 fixtures 在 `examples/shared` 里已经有。

之后是 **G05 OA 第一切片**——它一条解三处（H02 的完整版、G06、G10 都等着它），
