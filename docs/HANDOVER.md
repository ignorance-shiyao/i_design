...
- 本轮已完成并勾掉的：B01 AppShell、B03 QueryFilter、B04+B05 ProTable、
  B07 批量操作栏、B09 表单页形态、B10 人员选择、B06 树表、B11 导入向导与导出任务、B12 详情页、
  F06 运行状态、F08 确认失效、
  B08 SchemaForm、C03 图标填充变体、C06 插画矩阵、D04 分布图、D13 轴系、
  G02 示例门户、G03 ERP 第一切片（本轮换成组件后转正）。

**下一件事**：**B14 / B15 / B18 的补完** —— 评论的未读定位、看板与人员泳道的
拖动、异步任务中心；三条都是 🟡，各缺一块明确的东西。
B06 的树表与分组汇总已完成，只剩多级表头与单元格合并没做——
那是表头的结构问题，不是行的状态问题，排在这三条之后。
