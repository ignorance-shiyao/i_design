...
- 本轮从最新 `main` 的 `5634fcb0` 开始，工作分支为 `feat/h02-business-e2e`。
  H02 回归、Agent 乱序确认修复、单测源码入口修复与旧令牌删除分别提交。
  合并状态以 Git 为准，不再引用旧会话分支作为 main 的位置。
...
- **复杂表格**：B06 整条完成——多级表头由列树推导跨列/跨行，纵向合并只认
  ...
  （合并是排版结果，不是数据上的分组关系）。
- **智能体**：F06 运行状态、F08 确认失效、F10 产物工作区（含版本间的逐行差异：
  只跟紧邻旧版比，结构化产物只报规模变化，不假装做单元格级比对）。
- **视觉与图表**：C03 图标填充变体、C06 插画矩阵、D04 分布图、D13 轴系。
- **示例**：G02 示例门户、G03 ERP 第一切片、G08 Agent 第一切片
  （四种结局同一条管道、都可重放；运行停在人类确认那一步等人点）。

**G05 OA 第一切片已完成**：提交报销 → 审批人待办 → 退回修改 → 原单重提 → 通过 → 通知。
状态、权限、版本、幂等与通知事务在 `packages/common/src/logic/oa.ts`，
界面复用现有表单、人员选择、详情和任务中心。9 条逻辑测试；见 `docs/G05-OA.md`。
数据为明确标注的内存示例，刷新重置，不代表真实后端或移动运行时已验证。

**H02 已覆盖 ERP / OA / Agent 第一切片**：15 条浏览器业务路径，
4 项构建代码故障注入，已接入 CI；OA 额外量四种页面状态的宽窄屏与亮暗态无障碍。
详见 `docs/H02-E2E.md`。本轮工作分支为 `feat/g05-oa-workflow`，基于 H02 分支继续。

**H04 本轮补项已完成**：固定 100 节点画布与 1000 条会话负载，实测 642/13011 元素；
6 次真实浏览器源码故障注入，显式 GC 后三次 JS heap 采样。见 `docs/H04-PERF-BASELINE.md`。
开发分支 `feat/h04-perf-baseline`，承接 `feat/g05-oa-workflow`。
第 10 节剩余的 H03/H06/H07 需要外部环境或权限，按该节说明处理。

**B16 已完成本轮页面配方**：`/#/design/access`，模拟登录、个人设置、用户角色/组织/状态调整、
固定角色权限、组织创建与改名。common 唯一权限矩阵与事务模型；8 条逻辑测试，
`check:access` 覆盖业务结果与 10 次真实源码故障注入，已接 CI。见 `docs/B16-ACCESS-PATTERNS.md`。
分支 `feat/b16-access-patterns` 承接 `feat/h04-perf-baseline`；远端 main 核对仍为 `5634fcb0`。
**B17 已完成公共网页配方**：`/#/design/public`，Hero、功能区、示例定价、FAQ、
帮助搜索/文章与更新日志。地址保留栏目、计价和搜索状态，支持刷新及前进后退；
真实组件入口可访问，价格与权益明确为虚构示例，不连接交易。6 条 common 单测，
`check:public` 浏览器闭环与 11 次源码故障注入，已接 CI。见 `docs/B17-PUBLIC-PATTERNS.md`。
分支 `feat/b17-public-patterns` 承接 `feat/b16-access-patterns`。
**D05 帕累托图已完成本轮切片**：`/#/components/pareto`，`IChartPareto` 各端齐备。
统计/排序只在 common 的 `buildPareto`，所有渲染端消费同一个可序列化模型；
Flutter 不新增第二份统计算法。6 条逻辑测试，`check:pareto` 浏览器实测与故障注入，见 `docs/D05-PARETO.md`。
分支 `feat/d05-pareto` 承接 `feat/b17-public-patterns`；小程序与 Flutter 仍未做真机验证。
**D05 已全部完成**：帕累托、层级三件套、留存、贡献/流量平衡，见
`docs/D05-BALANCE.md`。留存的分母恒为本批期初、未到期是空格不是 0、
右边几期的平均标明「几个批次算的」；贡献那张的负值是数据不是脏数据，
对不上的差额单列成「未解释的差额」，不摊进最后一根柱子。

**层级三件套**：共享汇总契约 `logic/hierarchy.ts` + 旭日 + Icicle，
各端齐备，见 `docs/D05-HIERARCHY.md`。父子差额单列为「未细分」而不是替读者挑一个数，
...
分支 `cursor/d07-realtime-window-c2d1`。

**下一件仓库内可做的是 D08 邻接矩阵**，不依赖别的人或环境。
难点在排序口径（按度数、按社群还是按输入序）与稀疏矩阵里空格的语义
（没有边，还是没统计到）——和留存里「未到期不是 0」、断流里「空档不是 0」
是同一类问题：口径不写死在共享逻辑里，两个端迟早各算各的。

**H03 剩下的两件、H06、H07 在这个仓库里做不完**，别反复尝试：截图基线必须在
跑 CI 的那个环境里生成（开发容器字体不同，从这里提交会让 CI 全红）；人工读屏抽检
要人来做；H06 卡在用户手动配 NPM_TOKEN，**在它做完之前任何地方都不要宣称「可发布」**；
H07 要真机或模拟器，**缺真机验证就不得标 runtime-verified**。

其余还开着的口子，按块记在 `docs/astra.md` 第 10 节的状态表里：
D08（邻接矩阵）、E02/E03/E04/E09/E10（节点编辑器那一族）、
F05（附件校验与草稿恢复）、G12（页面模板导出）。
