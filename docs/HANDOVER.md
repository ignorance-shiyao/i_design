...
- 本轮已完成并勾掉的：B01 AppShell、B03 QueryFilter、B04+B05 ProTable、
  B07 批量操作栏、B09 表单页形态、B10 人员选择、B11 导入向导、B12 详情页、
  F06 运行状态、F08 确认失效、
  B08 SchemaForm、C03 图标填充变体、C06 插画矩阵、D04 分布图、D13 轴系、
  G02 示例门户、G03 ERP 第一切片（本轮换成组件后转正）。

**下一件事**：B11 的另一半 —— **ExportJob**（导入那一半已完成）。
导出是「任务」而不是「下载」：几万行要排队、要能看进度、要能取消，
生成好的文件还有有效期。难点是「这份导出是什么时候、按哪套筛选条件生成的」——
没写清楚的话，两个人拿着两份不同的表对不上账。之后是 B06 树表。

F06、F08、B07、B09、B10、B12、G03 与 B11 的导入那一半已完成
（见下面「这一轮做过什么」）。
