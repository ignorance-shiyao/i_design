...
- 本轮已完成并勾掉的：B01 AppShell、B03 QueryFilter、B04+B05 ProTable、F06 运行状态、F08 确认失效、
  B08 SchemaForm、C03 图标填充变体、C06 插画矩阵、D04 分布图、D13 轴系、
  G02 示例门户、G03 ERP 第一切片（判 🟡，原因见下）。

**下一件事**：`docs/astra.md` §10 的「下一步的顺序」第 6 条 —— **B09 表单页形态**
（FormPage / DrawerForm / ModalForm / StepForm）。B08 SchemaForm 已完成，
它的前置早就解除了，而它同时卡着 G03 的「新建 / 编辑」两页收口——
那两页现在是手写的。之后是 B07 批量操作栏与 B12 DetailPage，
三条做完 G03 才谈得上从 🟡 转正。

F06 与 F08 已完成（见下面「这一轮做过什么」）。
...
G03 判 🟡 而不是 ✅ 的原因：它能跑，但页面是手写的，还没换成 B04/B07/B09/B12 的组件。
现在 B04/B05 已完成，把 ERP 列表换成 `IProTable` 是一件随时可做的收口工作。
