...
**H02 第一版端到端回归已完成**：`npm run check:e2e` 构建示例后，
用 Playwright 验证 ERP 订单链与 Agent 四种结局 × 三种投递方式，各局再重放一次。
检查最终业务结果，并通过修改真实构建代码证明故障会判红，已接入 CI。
ERP 的权限与版本冲突由界面阻断写入；这不宣称浏览器收到了 HTTP 403/409。
OA 尚未覆盖，H02 保持部分完成。详见 `docs/H02-E2E.md`。

**下一件事：G05 OA 第一切片**——它一条解三处（H02 的完整版、G06、G10 都等着它），
组件前置（B09/B10/B12/B18）全部就位，卡的只是这条业务链本身没搭。
再往后是 **H04 的补完**（画布与长消息列表、内存，都要先想清楚拿什么当那个稳定的量）。
