...
- 本轮已完成并勾掉的：B01 AppShell、B03 QueryFilter、B04+B05 ProTable、
  B09 表单页形态、F06 运行状态、F08 确认失效、
  B08 SchemaForm、C03 图标填充变体、C06 插画矩阵、D04 分布图、D13 轴系、
  G02 示例门户、G03 ERP 第一切片（判 🟡，原因见下）。

**下一件事**：`docs/astra.md` §10 的「下一步的顺序」第 7 条 —— **B07 批量操作栏**。
`ITable` 有行选择，但没有「当前页 / 跨页 / 全部匹配」的作用域与部分失败定位。
三种作用域的语义差别是这条的全部难点：用户以为勾的是当前页，实际发出去的是
「全部匹配」，那是一次谁也收不回的批量操作。之后是 B12 DetailPage；
两条做完，G03 里手写的那几页才换得掉。

F06、F08 与 B09 已完成（见下面「这一轮做过什么」）。
