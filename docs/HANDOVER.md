...
- **壳与列表**：B01 AppShell、B03 QueryFilter、B04+B05 ProTable、B07 批量操作栏、
  B06 树表与分组汇总（多级表头与单元格合并未做）。
- **表单与详情**：B08 SchemaForm、B09 表单页四形态、B10 人员 / 组织选择、B12 详情页。
- **数据进出**：B11 导入向导与导出任务。
- **协作**：B14 评论线程与未读定位、B15 看板与人员泳道、B18 异步任务中心。
- **智能体**：F06 运行状态、F08 确认失效。
- **视觉与图表**：C03 图标填充变体、C06 插画矩阵、D04 分布图、D13 轴系。
- **示例**：G02 示例门户、G03 ERP 第一切片（换成组件后转正）。

**下一件事**：**B06 剩下的多级表头与单元格合并**。树表、分组小计、
折叠不丢选择都已经立住，剩下的是**表头的结构问题**——跨列的表头怎么排、
合并单元格怎么与排序共存（合并跨越的那几行被排序打散之后，合并还成不成立）。
它与「行的状态」是两类问题，所以单独排一档。
