...
Thinking 的分步轨迹、PromptInput 的 `@` 与 `/`）都已补齐，B 组任务全部完成。
...
- [x] **D6 · 翻页、窄屏自适应、侧栏分组粘顶**（P1）
  - **每页底部的上一页 / 下一页**。窄屏上侧栏是收起来的抽屉，读完一页想看
    下一个组件，得先点开抽屉、在几十个入口里找到自己刚才那一项、再点它下面
    那一个——而「下一个」这件事本身不需要一次查找。翻页跨组连着走
    （一组的最后一页接下一组的第一页，按组切断的话读到组尾会突然没有下一页，
    而侧栏里明明还有内容），两侧都写上分组名，否则跨组那一跳看起来像跳到了
    不相干的地方。到头的那一侧留空而不是放灰按钮：点不动的按钮要试一下才知道
    没有下一页。挂在 `DocLayout` 上而不是每页各写一遍——一百多个页面，
    漏掉一个不会报错，只会让那一页成为读者的死胡同
  - **侧栏分组名粘在顶部**。侧栏内容三千多像素、可视只有八百，滚到中段时
    屏幕上只剩一列组件名，「我现在在哪一组」得往回滚才知道。顺带把上边缘的
    渐隐去掉了：渐隐会把粘住的标题一起淡掉，看起来像这行字正在消失
  - **窄屏自适应**：390px 下 3 页横向溢出、320px 下 7 页。逐个查到根因后
    分两类修——组件层（分段控制器 `width: fit-content` 永不缩小、区间输入与
    输入框忘了 `min-width: 0`、验证码格子写死 44px、左标签表单在 480px 以下
    改成上标签、长路径不折行），以及演示页层（`width: NNNpx` 改成
    `width: min(NNNpx, 100%)`、`minmax(NNNpx, 1fr)` 改成
    `minmax(min(NNNpx, 100%), 1fr)`）
  - 新增 `check:responsive` 并接进 CI：两个宽度各扫一遍全站，
    整页能左右拖就算失败，并点名是哪个元素。判定用的是
    `scrollWidth > innerWidth` 而不是「某元素比容器宽」——宽表格与代码块
    本来就该自己横向滚动，那是有意为之。故障注入验证过：给一页塞
    `min-width: 900px` 后两个宽度都报红并点出了 `div.w`
  - 实测（Playwright）：翻页在组中、组尾、首页、末页四种位置都正确，
    点下一页确实跳到 `/components/divider`；侧栏滚到 900 粘住「导航」、
    滚到 2200 粘住「数据展示」；两个宽度全站零溢出

- [x] **D7 · 侧栏定位、调参面板分档、代码片段排版**（P1）
  - **侧栏顶上那条白带删掉**：顶部留白原先写在 `.doc-layout` 上，两列一起下移，
    于是侧栏顶上空出一条什么也不承载的白，看起来像菜单没对齐。改成只给正文那一列，
    侧栏的第一行就是搜索框、紧贴页头。页头高度提成 `--doc-header-h`，
    两处各写一个数字迟早会对不上
  - **搜索框与分组标题都粘住**：搜索框粘在侧栏顶、分组标题粘在搜索框下沿。
    侧栏三千多像素的内容，滚到中段想换个关键词就得先滚回顶部，
    而「搜一下」恰恰是内容多了之后最常用的入口
  - **调参面板分档**：按控件形态分成「外观 / 内容 / 开关」三档，
    不靠手维护一份属性名清单——手写清单会漂移，且漂移不会让任何构建失败，
    只会让新属性悄悄落到错误的一档。属性名改到控件正上方：左右分开时各个控件
    宽度差得很远（两个选项的分段器与一个开关差着三倍），每行的控件各自停在
    不同位置上，看起来像和左边的名字没关系；说明文字也终于有整格宽度，
    不必截成「视觉层级：主按钮…」
  - **代码块的两处排版缺陷**：
    一是 `<pre>` 里的模板空白被原样渲染——每个 `</span>` 都另起一行，
    于是每行代码后面跟着一个换行加八个空格，表现为行距凭空翻倍、上下各空一截
    （实测行盒高 44px，行高只有 22px）。二是示例都写在页面模板内部，
    带着页面那几层缩进，读者照着抄下来还得先手工对齐。补了公共层的
    `dedentCode`（按最小缩进裁，不逐行 trim，否则代码本身的层级也被抹平），
    四端共用
  - **片段放得下就写一行**：此前无论长短一律拆成多行，一个属性的按钮也要占四行。
    宽度按**显示列数**算而不是字符数——这是中文文档站，`提交` 两个字各占两列，
    按 `.length` 算会把一行 100 列宽的代码判成还放得下
  - 实测（Playwright）：侧栏滚动后与页头间隙为 0、搜索框与分组标题同时粘住且不重叠；
    调参面板三档正确、属性名不再铺满整格；代码块行盒高回到 22px、
    多行示例缩进保留、鼠标选中复制仍能拿到换行；单元测试 158 → 176，
    golden 断言 1451 → 1457；无障碍与窄屏两项检查均通过
...
- [x] **A4 · FloatButton 悬浮操作按钮**（P2，已完成）
  ...
  - 组件全景页当时只剩会话列表（B11）一项名副其实的 `planned`；B11 落地后归零
...
- [x] **B8 · Selection Actions 选区操作**（P2）：选中一段文字就地交给智能体改写
  - 浮条跟着选区走，不放在页面角上：选完再把视线挪到别处去找入口，中途很容易
    碰一下页面把选区清掉，那时用户得重选一遍，而他并不知道自己做错了什么
  - 动作条不代替选择，只承接选择：Web 与 Flutter 都走系统自己的选字行为
    （Flutter 用 `SelectionArea` 的 `contextMenuBuilder`），这个库只替换
    选完之后弹出来的那排动作。自己造一套选择手柄必然与系统不一致，
    而用户对选字的肌肉记忆来自系统而不是这个库
  - 小程序没有任何接口能读回用户划到了哪几个字（`user-select` 只让系统能复制），
    所以那一端按段落选：长按一段把整段交给动作条。差别在「怎么选」，
    不在「选中之后怎么办」——清理、上限、引文省略、摆哪边都是同一份逻辑
  - 换行换成空格而不是删掉（删掉会把「the quick\nbrown」粘成「quickbrown」），
    段落间的空行保留——那是作者的分段；引文从中间省略，掐尾会让好几段不同的
    选区看起来一模一样；超过上限时给一句「选短一些」，而不是一排点不动的灰按钮
  - 动作条默认开在选区上方（刚划过的那片正被手遮着），上方放不下才翻下去，
    判定复用 `shouldFlipUp` 的同一套算法
  - 四端齐备，golden 断言 1326 → 1358，单元测试 +9
  - 实测（Playwright）：拖选一段 → 浮条出现在段落上方且不越界、引文与三个动作
    正确 → 点「改写」拿到清理后的文本且浮条收起 → 重选后按 Esc 收起，无控制台报错
- [x] **B9 · Fine-tune Card 属性检查器**（P2）
  - 与一张普通表单差在一件事上：随时看得出「哪几项被我改过」，并且能单独退回去。
    没有这条，用户调了七八下之后就不敢再动了——他不知道自己已经偏离原始结果多远，
    也不知道怎么退回某一项。所以这一层的函数都围绕「当前值与原始值的差」写
  - 改动给三条线索：改过的那一项旁边多出一个**写着原值**的退回按钮、
    标题上写着「改了 3 项」、行底垫一块淡色。颜色是最弱的那条
  - 没改的项上不摆灰按钮：那样整列看起来全是可退回的，读者得逐个看清楚
    才知道自己动过哪些
  - 夹范围与吸步长在逻辑层做，不交给输入控件：滑块自己会吸附、数字框不会，
    同一个属性用拖的得到 12、用敲的得到 12.7，保存下来就不是同一份配置；
    浮点步长按步长的小数位收尾，不留 0.30000000000000004
  - 零项时说「与原始结果一致」而不是「改了 0 项」——后者要读者自己把 0
    翻译成「没改」，而这行字存在的意义正是省掉这一步
  - 四端齐备，golden 断言 1358 → 1379，单元测试 +9
  - 实测（Playwright）：敲 23 吸附成 24 并出现写着「16px」的退回按钮 →
    再改开关变成「改了 2 项」→ 单项退回剩 1 项 → 全部退回后按钮重新置灰。
    行高原先因说明折三行而参差（36/48/36/36 之前是 36/76/36/36），
    已把标签列加宽到能一行放下
- [x] **B10 · Agent Screen 智能体屏幕**（P2）：观看智能体操作屏幕，含工作中/加载中态
  - 这件事有一个不明显的难点：**一张静止的画面，看起来和一张卡住的画面
    一模一样**。智能体在想事情、网络断了、进程挂了——三种情况下画面都不动，
    而用户只能干等。所以花力气的地方不是画面本身，是围着画面的那几行字
  - 状态行写「在做什么」而不是「工作中」：后者没有信息量。暂停时把停在哪一步
    写出来（「已暂停：等待验证码」）
  - 画面带时间戳，超过 15 秒没动就在**画面上**压一条提醒，而不是挤进头部那一行——
    用户此刻正看着画面。做完之后不提醒，那张画面本来就不会再变
  - 「多久没动」与「什么时候的」是两句话：混用会写出「画面已经 44 秒前没动了」，
    而「44 秒前」是一个时刻不是一段时长。两句各有一个函数，都在公共层
  - 两秒内不报新鲜度：每一帧都挂「刚刚更新」的话那行字就是噪声，
    真卡住时反而没人注意到它变了
  - 高宽比在第一帧之前就定下来，否则连上的那一刻整页会跳，而那一刻用户正盯着看
  - 只有正在做事或已暂停时能接管：没连上时没东西可管，做完或出错后接管
    等于重新开一局，那是另一个按钮的事
  - 四端齐备，golden 断言 1379 → 1413，单元测试 +8
  - 实测（Playwright）：三块屏的状态行、时间戳、接管可用性均正确，
    停 44 秒那块压出提醒且文案是「画面已经 44 秒没动了」；秒表确实在走
    （5 秒前 → 7 秒前）；画面框高宽比稳定为 1.6，390px 窄屏不横向溢出
- [x] **B11 · 会话列表**（P2）：多轮会话的切换与管理
  - 按时间分「今天 / 昨天 / 最近 7 天 / 更早」四档：再细分，读者得先读组标题
    才知道自己在看哪一段。空组不渲染标题——一条也没有的组标题只是在占地方。
    置顶优先于时间，置顶的意思就是「别让它沉下去」
  - 分组按**天数**而不是按 7×24 小时：昨晚 23 点的会话，今早不该因为差了
    几小时就掉进「更早」
  - 标题为空时用摘要顶上（智能体还没起名），都没有才叫「新会话」——
    显示一片空白的话，用户会以为这条会话坏了
  - **删掉当前这条之后选它的后一条**，没有后一条才退到前一条。而且「后一条」
    按**分组压平之后的顺序**算：实测时按原数组算，删列表中段的「季度数据核对」
    跳到了顶部的「常用提示词」——置顶的会话显示时被提到最前，在原数组里
    还待在原位，两套顺序从来对不上。上下键本来就走压平顺序，这两处现在一致了
  - 搜索同时看标题与摘要：用户记得的往往是自己说过的那句话，而不是智能体
    起的标题。搜索框超过一定条数才出现，少几条时那个框只是占地方
  - 顺手补了一个 `pin` 图标：此前置顶标记借用勾、置顶按钮借用文件夹，
    两个都读成了别的意思。图标 52 → 53
  - 四端齐备，golden 断言 1413 → 1451，单元测试 +8
  - 实测（Playwright）：五组分组正确；搜「结论」只命中摘要里有它的那条；
    删当前选中后落到「上周那张图」（修复前落到「常用提示词」）；上下键、
    置顶、空态文案均正确
  - **组件全景页的规划中条目归零**：`check-catalog` 里两处以 `IChatList`
    为「还没实现的那一项」的断言改成自己造一个条目来试，否则这条路径
    再也走不到，等于悄悄失效
