...
缺的是**编辑体验**里剩下的两项：连线锚点选择、节点分组。任务见 C 组。
...
- [ ] **E10 · 清理无障碍存量**（P2，来自 E6；关键级已清零，剩下的是品牌决策）
  - 119 → 84 → 49 → **33**。做法始终是**先找根因再动令牌**，不逐处改样式
  - 第一轮（对比度的大头）：三个源头——第三级文字色（#8a8e99 在白底 3.28:1，
    全站最小的字恰恰最难读）、把填充色当文字写在同色淡底上（标签的「可用」
    在全景页出现近百次，2.03:1）、链接与文字按钮直接用品牌色（3.86:1）。
    新增 `color-{success,warning,danger,brand}-text` 四个令牌，只做一件事：
    在对应淡底上稳定过 4.5:1
  - 第二轮（32 项 critical + 5 项 serious）：**控件没有名字**——读屏用户听到的是
    「开关，未选中」「编辑框，空」，开的是什么、填的是什么全靠猜。这比对比度更要命：
    对比度差还看得见，没有名字等于这个控件不存在。根因在库里：`role="combobox"`
    的内容被读成「当前值」而不是「这是什么」；Switch 只有一个圆点、Slider 只有
    一个把手，本来就没有可读内容。补 `ariaLabel`，Vue 与 React 同步。
    另有真 bug：`ITextarea`/`IInputNumber` 把外部属性落在包装 div 上，
    `<IFormItem>` 给的 `id` 挂错元素，`<label for>` 指向一个不可标注的东西
  - 第三轮（对比度剩余）：根因是「填充色被拿来写字」——`color: var(--i-color-brand)`
    这类写法在共享样式里 118 处、站点里 25 处。四个 `*-text` 令牌本来就是为这件事
    加的，只是没人用。一次性换过来，顺带把代码注释色从 2.4:1 压到 4.8:1。
    主题面板那一层还有个真 bug：`resolveThemeTokens` 把 `color-text-link` 直接设成
    品牌色，且完全不产出四个 `*-text` 令牌——换成红色主题，填充跟着变，链接却还是
    编译时按原品牌色算出的深蓝。新增 `readableOn()`：只动明度、不动色相，
    把颜色调到在自己的淡底上过 4.5 为止（淡底过了，白底自然也过）
  - 剩下 33 项全是「白字压在饱和底色上」（品牌蓝 3.86、危险红、警告橙）。
    **这一项不能靠换文字色解决**：`CLAUDE.md` 明确规定饱和底色上一律白字，
    深字在中等明度底色上看起来像没渲染完。要过 4.5 只能压深品牌色本身，
    那是品牌决策，不该由实现顺手改掉——留给产品定
