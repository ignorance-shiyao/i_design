...
- [ ] **E10 · 清理无障碍存量**（P2，来自 E6；关键级已清零）
  - 已清：119 → 84 → **49**。第二轮清掉的是 32 项 critical 与 5 项 serious，
    也就是**控件没有名字**那一类：读屏用户听到的是「开关，未选中」「编辑框，空」，
    开的是什么、填的是什么全靠猜。这比对比度更要命——对比度差还看得见，
    没有名字等于这个控件不存在
  - 根因在库里而不在示例页：`role="combobox"` 的内容被读成「当前值」而不是
    「这是什么」，所以 Select 的触发器必须另给名字（现默认用占位文案）；
    Switch、Slider 只有一个圆点或一个把手，本来就没有可读内容；
    Checkbox 在表头全选、表格行首那几处没有可见文字。这些都补了 `ariaLabel`，
    Vue 与 React 两端同步
  - 另外两处是真 bug：`ITextarea` 与 `IInputNumber` 把外部属性落在包装 div 上，
    于是 `<IFormItem>` 给的 `id` 挂错了元素，`<label for>` 指向一个不可标注的
    东西——标签白写了。改成 `inheritAttrs: false` 并把属性转给里面的控件
    （class 与 style 仍留在外层）
  - 站点参数面板一次修好一片：控件左边虽然写着属性名，但那是个独立的 div，
    读屏不会把它和控件关联起来
  - 剩下 49 项：43 项对比度、4 项可滚动区域不可聚焦、2 项文字块里的链接只靠颜色区分做法不是逐处改样式，而是**先找根因再动令牌**——
    axe 在六个页面上数出的 250 多处对比度问题，实际只来自三个源头：
    第三级文字色（#8a8e99 在白底 3.28:1，全站最小的字恰恰最难读）、
    「把填充色当文字写在同色淡底上」（标签的「可用」在全景页出现近百次，2.03:1）、
    以及链接与文字按钮直接用品牌色（3.86:1）
  - 新增 `color-{success,warning,danger,brand}-text` 四个令牌，只做一件事：
    在对应淡底上稳定过 4.5:1。换主题色时跟着一起推导，不必每处手挑深色
  - 对比度那 43 项里最大一块是「白字压在品牌蓝上」（3.86:1）。
