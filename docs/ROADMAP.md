...
本文件保留历史实施记录与设计理由，不再维护第二份待办状态。
**当前任务、优先级和验收只在 [主台账](当前现状&后续规划_Claude%20.md) 第 2 节维护，一项任务一个 PR。**
下文的「转入」指向主台账编号，不是另一份待办。F1 逐条核对了原来的 32 个未勾选项；
源码存在与行为完成分开判断，原要求未满足的细节继续保留，不能仅凭组件文件勾选。

规模、共享逻辑清单与本次完成项的源文件证据见 [自动生成的源码快照](SOURCE_STATUS.md)。
F0 的 ready / planned 只表明示例源码存在，不代表包可安装、各端属性一致或整条任务验收通过。
...
- [x] **补动效令牌**：已在现有 `--i-motion-fast/base/slow` 之外补齐进出场曲线
      （`--i-motion-easing-in` / `-out` / `-spring`），并说明各自的用途
- 转入 **A3**：**列表进出场**：表格行、通知、附件条、消息用统一的 `TransitionGroup` 模式，
      含 `-move` 过渡，删除中间项时其余项平滑补位而不是瞬移
- 转入 **A3**：**折叠与展开**：Collapse、Menu 子级、Tree 分支统一用 grid-template-rows 过渡，
      高度未知时也能动（`max-height` 猜值会在长内容上突变）
- 转入 **A3**：**按压反馈**：按钮、菜单项、单元格的 `:active` 给一次极短的缩放，
      移动端尤其需要——没有 hover，按下去没反馈就像卡住
- 转入 **A3**：**骨架屏与加载态**：扫光方向与阅读方向一致
- 转入 **A3**：**全部尊重 `prefers-reduced-motion`**，并接入主题面板的动效开关
...
已有但能力不足（状态只在主台账更新）：
- 转入 **B2**：**Loading State**：现有 ILoading 只有转圈。补「已耗时」计时与多种律动形态——
      智能体等待动辄十几秒，不显示已等多久，用户会以为卡死
- 转入 **B3**：**Thinking 可展开轨迹**：现有 IChatThinking 只折叠一段文字。
      改为分步轨迹（推理 / 搜索 / 编码），每步可单独展开
- 转入 **B4**：**Prompt Bar 增强**：@ 引用来源、/ 命令、模型选择、语音输入

按「智能体工作流」的顺序保留完成记录与剩余项去向：
- [x] **Approval Card 征求确认**：智能体行动前的人类介入。单选/多选 + 自由输入，
      多问题时分页（1/3），可跳过。这是人机协作里最关键的一环
- [x] **Task Rows 任务行**：进行中 / 失败 / 完成的实时状态，可展开看细节
- 转入 **B1**：**Tool Chips 工具芯片**：把工具调用压缩成一行芯片，附带改动统计
...
- 转入 **B6**：**Insight Cards 洞察卡**：分页的洞察 + 可擦洗的实时图表（复用现有图表能力）
- 转入 **B8**：**Selection Actions 选区操作**：选中一段文字后就地交给智能体改写
- 转入 **B5**：**Code Block 代码块**：带行号的列表与统一 diff 视图（站点内已有实现，抽成组件）
- 转入 **B7**：**Command Search 命令搜索**：实时过滤 + 空状态，Cmd+K 唤起
- 转入 **B9**：**Fine-tune Card 属性检查器**：智能体调整设计属性时的面板
- 转入 **B10**：**Agent Screen 智能体屏幕**：观看智能体操作屏幕，含工作中/加载中状态
...
- [x] **Image 图片预览**：点击放大、滚轮缩放、多图切换、键盘左右
- [x] **Carousel 走马灯**：自动播放、指示点、触摸滑动。与移动端 Swiper 是同一个组件——
      翻页判定看位移**或**速度（只看位移会把手机上最自然的短促轻扫判成「没划够」），
      指示点超过上限只显示当前页附近的一段
- [x] **Calendar 日历**：按月查看与标记日程，含区间选择。复用 `logic/date`
- [x] **ColorPicker 颜色选择**：取色面板 + 预设色板。
      色彩换算直接用 `logic/palette` 的 OKLCH 实现，不再引第二套

## 第 5 批 · 布局与页面级

- [x] **Layout 页面框架**：顶栏 + 侧栏 + 内容区的整页骨架，与 Menu 配套
- 转入 **A5**：**Splitter 可拖拽分栏**：拖拽、最小宽度与键盘操作已实现，原要求的双击复位未实现
- [x] **Sticky 吸顶** 与 **BackTop 回到顶部**
- 转入 **A4**：**FloatButton 悬浮操作按钮**：Web 端缺失；移动端已有 IFab
- 转入 **A6**：**Watermark 水印**：平铺标记已实现，原要求的节点删除恢复未实现；前端水印不构成防泄露保证

## 第 6 批 · 图表补齐（ECharts / Highcharts 交集）

历史补齐范围如下，已完成项保留算法选择的原因：
...
历史选型补齐记录，以下完成项均已有实现；未完成范围已迁入主台账：
...
- [x] **Calendar 日历**：月视图与日期标记。标记用「淡底色块 + 文字」而不是给
      整格换底色——换底色会和「选中」「今天」抢同一个视觉通道；
      选完一段再点是重新开始选，从右往左点自动对调
...
- [x] **Mentions 提及**：`@` 唤起候选。三条判定缺一条都会让它在不该弹的时候弹出来：
      ...
      查询串有长度上限
- [x] **IndexBar 索引栏**：已有等价 IIndexes / Indexes，不能再建一套同义组件
- [x] **PullRefresh 下拉刷新**：已有等价 IPullDownRefresh / PullDownRefresh
...
- 转入 **E3**：**Input 的 `clearable`**：Vue 端有，React 端没有。Transfer 的搜索框因此
      两端都先不带清除按钮。补齐时顺带想清楚这类差异怎么自动检查——
      靠人工核对属性表，漏掉是必然的。

## 长期记录（已迁入主台账）

- [x] **Mention 提及**：已由 IMentions 实现，与第 9 批为同一项
- 转入 **B11**：**会话列表**：多轮会话的切换与管理
- 转入 **E8**：Tooltip / Popconfirm 之外的浮层定位审计；已有部分组件共用定位，仍需逐端核对
...
5. 文档页 + 稳定组件标识及入口，运行 `npm run build:catalog`；任务验收后更新主台账
6. `npm run check:parity` 全绿，Playwright 实测关键交互

F1 核对结果：原 32 项中 11 项已有对应实现，21 项仍有未完成范围并迁入主台账。
其中 Splitter、Watermark 保留部分完成说明；骨架扫光已存在，但统一动效与开关联动仍待 A3。
