...
- [x] **AutoComplete 自动完成**：边输入边给候选，复用 `logic/select` 的键盘移动
      与 `logic/overlay` 的定位。只有真的高亮了某一条回车才算选中——
      没高亮时回车照常提交，否则用户敲的自定义内容会被静默替换成第一条候选
- [x] **TagInput 输入标签**：回车成标签、退格删末项（只在输入框已空时）、粘贴批量拆分。
      中途遇到分隔符时最后一段留在输入框里继续编辑；被拒绝时说出理由而不是静默丢弃
- [x] **TimePicker 时间选择**：时分秒与范围。范围按位判定而不是拼成完整时间再比大小——
      选择器是一列一列点的，拼起来比会让一个本来合法的小时因为分秒不合法而被禁掉；
      夹取时先夹范围再对齐步长，反过来会得到既不在范围也不在步长上的值
- [x] **Transfer 穿梭框**：两栏搬运、搜索、全选。搬走的要从勾选里清掉（否则会出现
      「已选 3 项」而屏幕上一个勾都没有）；表头全选只管当前可见的条目；禁用项不跟着搬
...
- [ ] **Input 的 `clearable`**：Vue 端有，React 端没有。Transfer 的搜索框因此
      两端都先不带清除按钮。补齐时顺带想清楚这类差异怎么自动检查——
      靠人工核对属性表，漏掉是必然的。
