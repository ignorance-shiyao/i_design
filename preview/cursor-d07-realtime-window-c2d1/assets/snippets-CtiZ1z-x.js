const t={button:{"vue-next":`<script setup lang="ts">
...
</template>`,react:`import { Button } from '@i-design/react'
...
<i-button variant="secondary" loading="{{saving}}">保存</i-button>`,"mobile-vue":`<script setup lang="ts">
...
</template>`,"mobile-react":`import { Button } from '@i-design/mobile-react'
...
IButton(
  ...
  onPressed: onSubmit,
  child: const Text('提交'),
)`},select:{"vue-next":`<script setup lang="ts">
import { ref } from 'vue'
...
<template>
  <ISelect v-model="city" :options="options" clearable />
</template>`,react:`import { useState } from 'react'
...
)`},chatMessage:{"vue-next":`<script setup lang="ts">
...
</template>`,react:`import { ChatMessage, ChatSuggestions } from '@i-design/react'
...
>
  {answer}
</ChatMessage>`,miniprogram:`<!-- index.json: { "usingComponents": { "i-chat-message": "/components/chat-message/index" } } -->
...
IChatMessage(
  ...
  child: Text(answer),
)`},promptInput:{"vue-next":`<script setup lang="ts">
import { ref } from 'vue'
...
<template>
  <IPromptInput
    ...
    @stop="stop"
  />
</template>`,react:`import { PromptInput } from '@i-design/react'
...
/>`,flutter:`IPromptInput(
  ...
  onSubmit: send,
  onStop: stop,
)`},pagination:{"vue-next":`<script setup lang="ts">
import { ref } from 'vue'
...
</template>`,react:`import { Pagination } from '@i-design/react'

<Pagination current={page} total={240} pageSize={10} onChange={setPage} />`,miniprogram:`<i-pagination
  current="{{page}}"
  total="{{240}}"
  page-size="{{10}}"
...
)`}},e=`// packages/common/src/logic/pagination.ts —— 多端共用的同一份规则
