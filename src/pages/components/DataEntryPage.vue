...
<ISlider v-model="threshold" :step="10" :marks="marks" />'
  ...
  <template #extra="{ item }"><ITag>{{ item.meta[2] }}</ITag></template>
    ...
    <p>
      三列并排而不是一个长下拉：时分秒拉成一条列表有八万多项，滚到想要的那一刻比手打还慢。
      「此刻」不是装饰——绝大多数时间输入填的就是现在，让人少滚三列。
    </p>
    <p>
      范围判定按位做，而不是先拼成完整时间再比大小。选择器是一列一列点的，
      用户点「小时」那一刻分和秒还是上一次的值；拼起来比较的话，
    ...
    <h2>Transfer 穿梭框</h2>
    <p>
      两处最容易各端给出不同答案、而两种答案都说得通的地方，都收进了公共层：
      搬走的条目要从勾选里清掉，否则会出现「已选 3 项」而屏幕上一个勾都没有，
      这个状态用户没法自己纠正；表头全选只管当前可见的条目，
      搜索状态下用户的意思是「这些」，把看不见的一起选中，
      再点搬运就会搬走一批他从没见过的条目。
    </p>
    <DemoBlock
      title="搬运、搜索与禁用项"
