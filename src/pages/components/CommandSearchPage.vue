...
<template>
  <article>
    ...
    <p class="i-lead">
      一个搜索框，一串实时过滤的结果，键盘全程可用。
      东西超过几十项之后，再怎么分组都不如直接搜——菜单、设置项、文档页都一样。
    </p>
    ...
    <h2>排序是怎么定的</h2>
    <p>
      搜索的成败全在排序：输入「按钮」，第一条必须是「按钮」而不是「按钮组」。
      所以匹配分三档，差距拉得很开——整串相等 &gt; 从头开始 &gt; 出现在词首 &gt; 出现在词中；
      字段再加权重，标题 &gt; 关键词 &gt; 描述，描述里命中只当补充证据，顶不到前面。
      同分时短标题排前面：它是这个词本身，而不是以它开头的另一件东西。
    </p>
    <p>
      这套规则放在公共层的 <code>searchCommands</code> 里，各端共用一份。
      各写一遍的话，同一个词在站点里与在应用里给出的第一条会不一样。
    </p>
