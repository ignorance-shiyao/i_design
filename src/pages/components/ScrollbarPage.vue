...
<template>
  <article>
    <h1>Scrollbar 滚动条</h1>
    <p class="i-lead">
      把系统滚动条换成一条不占布局的细条。Windows 的系统滚动条是一条十几像素宽的灰槽，
      会把右侧内容挤窄，而 macOS 上默认不占位——同一份布局在两个系统上看到的宽度不一样。
      滚动本身仍交给浏览器：滚轮惯性、触控板、键盘与读屏工具都指望原生滚动，
      这里只是把那条槽藏起来、另画一条。
    </p>
    ...
    <h2>各端差异</h2>
    <p>
      小程序端退回系统滚动条：那一端的滚动交给 <code>scroll-view</code> 承担，
      平台不允许把它的滚动条替换掉。外层的尺寸与留白规则一致，只是那条槽是系统画的。
      Flutter 端用平台自带的滚动条组件，滑块的最短长度与显形时机与这里一致。
    </p>
    <p class="sb-note">
      内容横向超出时仍由外层自行处理，这个组件只接管纵向——
      两个方向都接管会让「滚到哪了」出现两套判定，而横向滚动在文档里本就少见。
      需要横向滚动的表格请看 <RouterLink to="/components/table">Table</RouterLink>。
    </p>
  </article>
</template>
