...
<template>
  <article>
    <h1>Qrcode 二维码</h1>
    <p class="i-lead">
      把一段文本变成可扫的码。编码算法在公共层，各端共用同一份——
      同一段文本必须在 Web、小程序与 Flutter 上得到同一个版本与掩码，
      否则模块数对不上，设计稿里的尺寸与留白就得各端各调一遍。
    </p>
    ...
    <h2>为什么颜色不跟随主题色</h2>
    <p>
      扫描靠的是码与底的明暗差。主题色一旦落在中等明度，对比度就掉到相机读不出来，
      而这件事在设计稿上看不出来——图是清楚的，只是扫不动。
      因此这里只提供「深色码 + 浅色底」，不把主题色接进来。
    </p>
    ...
    <h2>容量</h2>
    <p>
      实现覆盖版本 1–10 的字节模式，版本 10 在 M 级下可放 213 字节。
      更大的版本模块数超过 57×57，印在屏幕上已经扫不动了——那时该换一种载体，而不是继续加版本。
    </p>
  </article>
</template>
