...
<template>
  <article>
    ...
    <h2>构成</h2>
    <p>
      比较各项大小用柱状图，且必须从零起——截断基线会放大差异，是最常见的图表误导。堆叠用于表达「合计由哪些部分构成」，相邻段之间留出底色缝隙，色觉障碍下也能分清边界。
    </p>
    <DemoBlock
      title="分组、堆叠与堆叠面积"
    ...
    <h2>占比</h2>
    <p>
      环形而不是实心饼：读者比较的是弧长，比面积更容易读准。数值写在图例上而不是扇区里，扇区一小就会互相压字。
    </p>
    ...
    <h2>转化</h2>
    <p>
      漏斗回答的是「在哪一步流失最多」，所以每层旁边写的是相对上一层的转化率，而不只是占起点的百分比。层宽按数值比例缩，不做等差递减——等差看着更顺，但那是画出来的顺，不是数据里的顺。层级有序，因此用单色阶而不是分类色。
    </p>
    ...
    <h2>单值与阈值</h2>
    <p>
      仪表盘用于「一个值离目标还有多远」。开口朝下的 270° 而不是整圆——整圆会让满值与零值落在同一个位置，无法分辨。越过阈值时整条弧转为对应的状态色。
    </p>
    <DemoBlock title="仪表盘">
    ...
    <h2>相关性</h2>
    <p>
      散点回答的是「这两个量之间有没有关系」。两个轴都要写清楚在量什么，只标数字读者不知道自己在看什么。气泡半径按<strong>面积</strong>映射而不是按半径——直接拿数值当半径，数值翻一倍看上去会是四倍大。
    </p>
    <p>
      散点的系列上限是 3，这不是随手定的：折线与柱状里只有相邻系列会挨在一起，散点则是任意两个点都可能贴着，所以配色必须按「所有两两组合」校验。本体系的分类色在这个更严的口径下，亮色与暗色两种模式都只有前三槽同时通过；第四槽与品牌蓝的常色差低于可分辨下限，色觉正常的人也难分。超出的系列会合并成「其他」，而不是再调一个颜色把问题藏起来。
    </p>
    <DemoBlock
      title="散点与拟合线"
    ...
    <h2>二维密度</h2>
    <p>
      热力图看「什么时候最忙」这类二维分布。用单色阶而不是彩虹：彩虹会让读者以为不同颜色代表不同类别，而不是多与少。深色格子上的数字自动转为反色，任何一格都读得出来。
    </p>
    ...
    <h2>流向</h2>
    <p>
      桑基图回答的是「量从哪儿来、到哪儿去、中途漏掉多少」。节点高度按流量占比分配，而不是等分——等分会让一条极小的支流和主干看起来一样粗，那正是它要避免的误读。
    </p>
    <DemoBlock
      title="桑基图"
    ...
    <h2>层级占比</h2>
    <p>
      矩形树图用面积表达占比，适合项数多到饼图已经切不动的场景。布局用 squarify 而不是简单切条——切条会产出又长又细的矩形，而人眼比较细长条的面积极不准。
    </p>
    <DemoBlock
      title="矩形树图"
    ...
    <h2>区间缩放</h2>
    <p>
      序列一长，整屏画出来就是一团噪声。缩放条给出全量的缩略走势，拖动窗口即可放大其中一段——窗口之外压暗而不是隐藏，用户才知道自己漏看了什么。
    </p>
    <DemoBlock
      title="拖动缩放"
    ...
    <h2>分布</h2>
    <p>
      箱线图回答的是「这批数据散得开不开、有没有异常」。它用单一色相而不是分类色——每个箱子都是同一件事（一个分布），给它们各上一个颜色会让人以为颜色本身有含义。
    </p>
    <DemoBlock
      title="箱线图"
    ...
    <h2>增减归因</h2>
    <p>
      瀑布图回答的是「从期初到期末，中间都发生了什么」。连接线把上一根的终点引到下一根的起点，这正是它区别于普通柱状图的地方。
    </p>
    <DemoBlock
      title="瀑布图"
    ...
    <h2>排期</h2>
    <p>
      甘特图回答的是「谁挡着谁、现在落后了没有」。因此依赖线与今天这条竖线是它的主体，横条只是把工期画出来而已。
    </p>
    <DemoBlock
      title="甘特图"
    ...
    <h2>词频</h2>
    <p>
      词云的编码是字号，不是颜色。它适合快速看出「哪几个词压倒性地多」，要拿确切数值仍然得看表——因此数据表是它的正式读法之一，而不是无障碍的补丁。
    </p>
    <DemoBlock
      title="词云"
    ...
    <h2>什么时候不该用它</h2>
    <ul>
      <li>只有两三个数、读者要的是准确值时——写成数字或一张小表，图表的坐标轴反而让人读不准。</li>
      <li>类别超过十来个时——先合并出「其他」，十几段颜色的饼图谁也分不清哪块是哪块。</li>
      <li>图形是唯一的表达时——每张图都要留数据表出口，颜色与形状在读屏与灰度打印下都不成立。</li>
    </ul>
  </article>
</template>
