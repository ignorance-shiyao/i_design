...
const fields = computed(() => visibleFields(props.schema, props.modelValue))

const errors = computed<FormFieldError[]>(() => [
  ...(touched.value ? validateSchema(props.schema, props.modelValue) : []),
...
watch(() => props.modelValue, () => (asyncErrors.value = []), { deep: true })
...
async function submit() {
  touched.value = true
  asyncErrors.value = []
  const sync = validateSchema(props.schema, props.modelValue)
  if (sync.length) {
    focusFirst(sync)
    emit('invalid', sync)
    return
  ...
  const results = await Promise.all(
    pending.map(async (rule) => ({
      rule,
      ok: await props.asyncValidator[rule.handler](props.modelValue[rule.path])
    }))
  )
  ...
  emit('submit', submitValues(props.schema, props.modelValue))
}
  ...
  <form ref="root" class="i-schema-form" novalidate @submit.prevent="submit">
    <div v-for="field in fields" :key="field.name" class="i-schema-form__field" :data-path="field.name">
      ...
      <IInput
        ...
        @update:model-value="(v: string) => setValue(field.name, v)"
      />
      ...
      <IInputNumber
        ...
        :model-value="(modelValue[field.name] as number | null) ?? null"
        :disabled="disabled"
      ...
      <ISelect
        ...
        :model-value="(modelValue[field.name] as never) ?? (field.kind === 'multi-select' ? [] : '')"
        :multiple="field.kind === 'multi-select'"
        :options="(field.options ?? []).map((o) => ({ value: o.value, label: o.label, disabled: !!o.disabledReason }))"
        :disabled="disabled"
        @update:model-value="(v: unknown) => setValue(field.name, v)"
      />
        ...
        <div v-for="(row, index) in rowsOf(field)" :key="index" class="i-schema-form__row">
          ...
          >
            ...
            <IInputNumber
              ...
              :model-value="(row[sub.name] as number | null) ?? null"
              :disabled="disabled"
              @update:model-value="(v: number | null) => setCell(field, index, sub.name, v)"
            />
            <IInput
              v-else
              :id="`f-${field.name}-${index}-${sub.name}`"
              :model-value="String(row[sub.name] ?? '')"
              :placeholder="sub.placeholder"
              :disabled="disabled"
              :invalid="!!errorOf(`${field.name}[${index}].${sub.name}`)"
              @update:model-value="(v: string) => setCell(field, index, sub.name, v)"
            />
