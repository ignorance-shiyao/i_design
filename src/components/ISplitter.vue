...
function apply(nextSize: number) {
  ...
  emit('update:modelValue', paneRatio(clamped, total.value, props.gutter))
}
...
function onKeydown(event: KeyboardEvent) {
  ...
  event.preventDefault()
  measure()
  apply(paneSize(props.modelValue, total.value, props.gutter) + step)
}
...
const firstStyle = computed(() => ({
  flex: `0 0 calc((100% - ${props.gutter}px) * ${props.modelValue})`
}))
