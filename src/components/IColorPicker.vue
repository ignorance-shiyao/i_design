...
const readout = computed(() => colorReadout(props.modelValue))
...
<template>
  <div ref="root" class="i-colorpicker" :class="{ 'is-disabled': disabled }">
    <button
      ...
      @click="toggle"
    >
      <span class="i-colorpicker__swatch" :style="{ background: modelValue }" />
      <span class="i-colorpicker__value">{{ modelValue }}</span>
      ...
      >
        ...
        <div class="i-colorpicker__row">
          ...
          <span
            ...
            :style="{ background: modelValue, color: readout.ink }"
          >
            {{ readout.ratio }}:1
          </span>
        </div>
        ...
        <div class="i-colorpicker__presets">
          <button
            ...
            :aria-label="preset"
            :aria-pressed="preset.toLowerCase() === modelValue.toLowerCase()"
            @click="emit('update:modelValue', preset)"
          >
            <IIcon v-if="preset.toLowerCase() === modelValue.toLowerCase()" name="check" :size="12" />
          </button>
        </div>
      </div>
    </Teleport>
