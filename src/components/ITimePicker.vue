...
const label = computed(() => (props.modelValue ? props.modelValue : ''))
...
function pick(unit: 'hour' | 'minute' | 'second', value: number) {
  ...
  draft.value = next
  emit('update:modelValue', formatTime(next, props.showSecond))
}
...
function now() {
  ...
  emit('update:modelValue', formatTime(draft.value, props.showSecond))
  scrollToActive()
}
        ...
        <div class="i-timepicker__cols">
          <ul v-for="col in columns" :key="col.unit" class="i-timepicker__col" role="listbox">
            <li
              ...
              role="option"
              :aria-selected="draft[col.unit] === value"
