...
const props = withDefaults(
  ...
  { modelValue: '', mode: 'single', marks: () => [], weekStart: 1, min: '', max: '' }
)

const emit = defineEmits<{ 'update:modelValue': [string | DateRange]; select: [string] }>()

const view = ref(parseISO(typeof props.modelValue === 'string' ? props.modelValue : props.modelValue?.start) ?? new Date())
...
const range = computed<DateRange>(() =>
  typeof props.modelValue === 'string'
    ? { start: props.modelValue || null, end: null }
...
function pick(iso: string) {
  ...
  focused.value = iso
  if (props.mode === 'range') {
    emit('update:modelValue', selectRange(range.value, iso))
  } else {
