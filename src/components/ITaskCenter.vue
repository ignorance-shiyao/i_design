...
const emit = defineEmits<{
  /** 用户要去看这条任务动到的业务对象 */
  open: [target: TaskTarget, task: AsyncTaskItem]
  /** 没有业务对象时点「查看任务详情」 */
  inspect: [task: AsyncTaskItem]
  retry: [task: AsyncTaskItem]
  cancel: [task: AsyncTaskItem]
  'update:tasks': [AsyncTaskItem[]]
}>()
    ...
    <ul v-if="ordered.length" class="i-task-center__list">
      ...
      >
        ...
        <div class="i-task-center__actions">
          ...
          <IButton size="sm" :aria-expanded="trailOpen(task.id)" @click="toggleTrail(task.id)">
